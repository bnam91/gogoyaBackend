        ...
        def normalize_brand(brand_name):
            ...
            if brand_info:
                return {
                    ...
                    'status': brand_info.get('status', 'ready')
                }
...
def normalize_brand(brand_name, brands_collection, author=None):
    ...
    try:
        ...
        if not brand_name or brand_name in ['None', 'Unspecified', '확인필요']:
            ...
            logger.info(f"{'='*50}\n")
            return {'name': '확인필요', 'category': '', 'level': '', 'status': 'ready'}
        ...
        if not similar_brands:
            ...
            logger.info(f"{'='*50}\n")
            return {'name': brand_name, 'category': '', 'level': '', 'status': 'ready'}
        ...
        if similar_brands:
            ...
            if analysis and analysis.get('representative_brand'):
                ...
                if rep_doc:
                    ...
                    # level 필드 유지 여부 확인
                    existing_level = rep_doc.get('level', '')
                    if existing_level == 'done':
                        level_to_set = 'done'
                    else:
                        level_to_set = ''
                    ...
                    brands_collection.update_one(
                        ...
                        {'$set': {
                            ...
                            'status': 'done',
                            'level': level_to_set  # level 필드 유지
                        }}
                    )
                    ...
                    return {
                        ...
                        'level': level_to_set,  # 유지된 level 필드 사용
                        'status': 'done'
                    }
                ...
                # rep_doc이 없는 경우의 처리 추가
                    ...
                    new_brand = {
                        ...
                        'status': 'done',
                        'level': ''  # 새로운 브랜드의 level은 빈 값으로 초기화
                    }
                    brands_collection.insert_one(new_brand)
